...
TEST(UpdateTest, CBID_Name) {
    ...
    EXPECT_TRUE(db.updateCBIDName("Doc", "9232") == 0);
...
TEST(UpdateTest, CBID_Nationality) {
    ...
    EXPECT_TRUE(db.updateCBIDNationality("Alien", "8607") == 1);
...
TEST(addNumber, CCND) {
    ...
    ASSERT_TRUE(db.addNumber("4305", "857363") == -1); //As we know 4305 already has 4 numbers registered so will return -1
...
TEST(checkRegistred_No, CCND) {
    ...
    ASSERT_TRUE(db.checkRegistred_No("9631") == 0);
...
TEST(AlienDatabase, AD) { //Test case to check if declared alien or not
    
    Database<string> db("CBID.txt", "CCID.txt", "CCND.txt");

    ASSERT_TRUE(db.declareAlien("7888").nationality == "Alien");
    ASSERT_TRUE(db.verifyAlienDBUpdate("7888") == 1);
    ASSERT_TRUE(db.verifyAlienDBUpdate("7564") == 0);
    db.Display_AD();
}
